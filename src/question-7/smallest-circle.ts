...
export function makeCircle<P extends Point>(
...
function makeCircleOnePoint<P extends Point>(
    points: Readonly<Array<P>>,
    p: Point,
): Circle {
...
function makeCircleTwoPoints<P extends Point>(
    points: Readonly<Array<P>>,
    p: Point,
    q: Point,
): Circle {
    ...
    for (const r of points) {
        ...
        if (c === null) continue;
            ...
            cross > 0 &&
            (left === null ||
                crossProduct(p.x, p.y, q.x, q.y, c.x, c.y) >
                    crossProduct(p.x, p.y, q.x, q.y, left.x, left.y))
        )
            left = c;
        else if (
            cross < 0 &&
            (right === null ||
                crossProduct(p.x, p.y, q.x, q.y, c.x, c.y) <
                    crossProduct(p.x, p.y, q.x, q.y, right.x, right.y))
        )
            right = c;
    }
...
function makeCircumcircle(a: Point, b: Point, c: Point): Circle | null {
    ...
    const ax: number = a.x - ox;
    const ay: number = a.y - oy;
    const bx: number = b.x - ox;
    const by: number = b.y - oy;
    const cx: number = c.x - ox;
    const cy: number = c.y - oy;
...
function isInCircle(c: Circle | null, p: Point): boolean {
    return (
        c !== null &&
        distance(p.x, p.y, c.x, c.y) <= c.r * MULTIPLICATIVE_EPSILON
    );
}
